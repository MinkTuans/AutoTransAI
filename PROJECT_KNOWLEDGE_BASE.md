...
1. **INGEST**: Import video, validate file/URL, probe media specs via FFprobe, store asset, extract WAV audio. Page URLs go through `PageURLAdapter` + yt-dlp (`POST /api/video-translator/import`). Supported page hosts include YouTube, Vimeo, TikTok, Facebook, Instagram, Google Drive, Dropbox, and Bilibili (`bilibili.com`, `b23.tv`). Bilibili check-url reads `https://api.bilibili.com/x/player/pagelist?bvid=` (duration in seconds per part; `?t=` is a timestamp, `?p=` is the part). The `view` API is often HTTP 412. Download still uses yt-dlp with browser User-Agent/Referer and `bv*+ba` DASH merge. yt-dlp subprocesses must run with `check=False` so stderr is mapped to a non-empty 400 `detail`.
