...
- `ProjectTerminologyMemory` (`project_terminology_memory`): AI auto-detected entity terminology terms with `source_term`, `suggested_term`, `term_type`, `confidence`, and `needs_review`. Studio Auto (`POST /jobs/{id}/start`) and TranslateStage persist after translation. Extraction reads both source (`text` / `original_text`) and `translated_text`. Heuristic keeps repeated CJK spans plus Unicode title-case names (Vietnamese diacritics included); LLM enrichment is optional and must not zero out heuristic hits.
