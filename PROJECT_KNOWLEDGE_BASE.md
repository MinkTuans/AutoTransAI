...
- **`video_translator/translator_service.py`**: Drives Phase 1 translation (FFprobe audio extraction, STT, diarization, terminology extraction, LLM translation, verbatim echo detection/recovery) and Phase 2 rendering. Direct Gemini and OpenAI STT functions accept optional request-scoped catalog targets and decrypted keys; the target's exact remote model reaches HTTP, while omitted targets retain legacy resolution. Gemini uses `x-goog-api-key` headers, not URL query credentials; provider response snippets are excluded from STT parser errors/logs. OpenAI STT non-200 responses preserve HTTP status in a sanitized `PipelineError` so route fallback classifies auth, model, capability, and rate failures correctly. The existing Studio translation loop still writes `_resolved_model_id` on registry providers and is pending Task 6B cutover.
