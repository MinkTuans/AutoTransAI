...
- **`video_translator/translator_service.py`**: Drives Phase 1 translation (FFprobe audio extraction, STT, diarization, terminology extraction, LLM translation, verbatim echo detection/recovery) and Phase 2 rendering. Studio Phase 1 STT now receives an async session factory and uses the canonical STT default through `build_route`/`invoke_route` when a non-system Gemini/OpenAI catalog model, Gemini/OpenAI canonical key, or refresh run exists. Its initial catalog session closes before provider calls; each target uses a separate short credential session and passes its exact remote model and decrypted key to the request-local Gemini/OpenAI STT adapter. Same-provider models/keys precede other providers regardless of the legacy fallback flag, and unsupported provider adapters advance with a capability mismatch. Missing/invalid STT defaults raise configuration errors; missing canonical tables surface migration errors. A schema without those STT activation signals uses legacy `.env` STT, so keyless system rows and unrelated TTS catalog/key rows alone do not activate cutover. Direct Gemini and OpenAI STT functions retain optional legacy calls outside this Studio entry. Gemini uses `x-goog-api-key` headers, not URL query credentials; provider response snippets are excluded from STT parser errors/logs. OpenAI STT non-200 responses preserve HTTP status for route classification. Studio translation/TTS, Unified, and public API/UI routing remain pending later Task 6 batches.
