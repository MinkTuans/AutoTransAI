...
3. **TRANSLATE**: Detect and normalize terminology, atomically write non-conflicting mappings into the project Glossary, load its canonical 1:1 mappings, translate with those rules in every retry path. Language-aware glossary enforcement: `find_glossary_violations` accepts `source_language` and `target_language`, automatically skips invalid self-mapped CJK entries (e.g. `安妮→安妮` when target is Vietnamese) via centralized `is_self_mapped_cjk()` utility while preserving legitimate same-term mappings (`AI→AI`, `Netflix→Netflix`). CJK self-mapped entries are filtered from LLM translation prompts, rejected during `persist_detected_terms`, and logged as `GLOSSARY_SKIP_INVALID_MAPPING`. Only valid glossary violations block DUB stage.
