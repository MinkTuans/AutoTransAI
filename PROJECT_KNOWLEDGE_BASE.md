...
- `ProjectTerminologyMemory` (`project_terminology_memory`): AI auto-detected **proper names only** (`character`, `location`, `organization`) with `source_term`, `suggested_term`, `term_type`, `confidence`. `needs_review` is not set on auto-extract (no confirm-into-glossary gate). Studio Auto (`POST /jobs/{id}/start`) and TranslateStage persist after translation. Extraction reads both source (`text` / `original_text`) and `translated_text`. Heuristic keeps repeated CJK names / place suffixes (`城`…) plus Unicode title-case names; CJK n-grams that contain function chars (`了的是我你…`) are dropped. LLM prompt forbids clauses. GET `/terminology-memory` filters the same way so leftover `Other` fragments are hidden.
