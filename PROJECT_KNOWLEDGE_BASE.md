...
1. **INGEST**: Import video, validate file/URL, probe media specs via FFprobe, store asset, extract WAV audio. Page URLs go through `PageURLAdapter` + yt-dlp (`POST /api/video-translator/import`). Supported page hosts include YouTube, Vimeo, TikTok, Facebook, Instagram, Google Drive, Dropbox, and Bilibili (`bilibili.com`, `b23.tv`). Bilibili uses browser User-Agent/Referer and `bv*+ba` DASH merge because the site WAF often returns HTTP 412 and does not ship a single `best[ext=mp4]` file. yt-dlp subprocesses must run with `check=False` so stderr is mapped to a non-empty 400 `detail` (empty `❌` previously rendered as `×` in the pipeline banner).
