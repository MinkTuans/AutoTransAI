...
3. **TRANSLATE**: Detect and normalize terminology, atomically write non-conflicting mappings into the project Glossary, load its canonical 1:1 mappings, translate with those rules in every retry path, then block DUB if canonical validation fails.
