...
- **Auxiliary terminology LLM route (Task 6E3A1)**: Studio Phase 1 and Unified TRANSLATE `extract_entities` now pass the canonical session factory to `extract_and_persist_from_segments`. If keyed/discovered Gemini/OpenAI/Anthropic LLM catalog state or a canonical translation default exists, terminology extraction uses `build_route(..., "LLM")` with the translation function default, exact request-local model/key and compatible same-provider/cross-provider candidates. Missing/invalid defaults and exhausted routes fail visibly; unsupported runtime adapters advance with a classified capability mismatch, while malformed JSON advances with `invalid_output`. Catalog and credential reads close before provider calls. The prompt is capped at 6 KiB or the smallest eligible persisted input-token limit minus a 512-token reserve, using UTF-8 bytes conservatively. No catalog state retains the legacy registry call as a migration hold until Task 9. Terminology rejection logs no longer include source or suggested term text. Editor QC/SEO, character mapping, and thumbnail auxiliary LLM calls are not cut over by this sub-batch.
