...
- **Legacy model identity import (2026-09-24, Task 9 data batch)**:
  - Added an explicit, caller-transactional import from archival `ai_models` to canonical `ai_catalog_models`. Exact provider/model identities are checked before writes; missing providers and collation-equal but byte-different identities stop the whole batch with fixed issue codes. Existing rows and original archive records are untouched on replay.
  - Imported rows use `legacy_import` and have no discovery evidence or key access. Catalog reads show them as unverified, and routing/readiness gates ignore them. A positive provider listing promotes the exact row to `discovered`; this does not activate or invent a credential. Synthetic SQLite tests cover import, replay, collision refusal and discovery promotion. Runtime cutover and configured databases remain outside this batch.

- **Thumbnail model-width migration prerequisite (2026-09-24, Task 9D8)**:
  - The published `20260923_thumbnail_model_length` revision now freezes the complete historical thumbnail table in a migration-owned helper. On a blank database it creates the current 255-character model column; on a complete 100-character historical table it validates the schema and dependencies before widening; a complete current table replays without writes. Partial or unknown schemas, views, triggers and incoming foreign keys that would make a SQLite rebuild unsafe are refused with fixed backup-reconciliation guidance.
  - Actual no-stamp Alembic `upgrade head` passes on disposable SQLite and MySQL 8.4. A populated historical thumbnail was widened on both engines with its row, foreign keys and indexes preserved. No configured database, live provider or key was used.

- **Frozen legacy catalog prerequisites (2026-09-24, Task 9D7 checkpoint)**:
  - The published `20260923_ai_catalog` revision now establishes the three pre-catalog tables `providers`, `ai_function_configs`, and `ai_models` from Git `08ea050^` before creating canonical tables. It accepts an all-absent or complete compatible historical group, preserves populated rows, and refuses partial/unknown schemas and occupied canonical names before online DDL. Offline SQL retains its prior assumption that legacy tables already exist; only online mode can inspect and create absent prerequisites. No mutable application service is imported by the migration.
  - Actual no-stamp Alembic traversal from base through `20260923_catalog_refresh` passes on disposable SQLite and MySQL 8.4. Updated the older catalog test fixtures to the complete historical prerequisite state. Startup-created current schemas remain separate Task 9 work; MySQL interruption can leave a partial prerequisite group requiring manual reconciliation.

- **MySQL Alembic version width (2026-09-24, Task 9D6 checkpoint)**:
  - The first published 33-character revision, `20260916_character_voice_timeline`, now validates and widens an Alembic-managed MySQL `alembic_version.version_num` from the default `VARCHAR(32)` to `VARCHAR(64)` after voice preflight and before its schema writes. A malformed version table or unknown stored revision refuses before voice changes; an existing 64-character column replays without DDL. Direct revision-operation tests without a version table remain supported. Disposable MySQL 8.4 RED→GREEN tests verify preserved predecessor, and an actual no-stamp Alembic upgrade from base through voice records the full 33-character ID. Adjacent migration tests pass; no configured database was used.

- **Historical foreign-key link (2026-09-24, Task 9D5 checkpoint)**:
  - Repaired published `20260918_add_fk` without changing its revision ID or ancestry. The historical `segments` project FK and optional startup-created `jobs`, `assets`, and `video_edit_configs` FKs are checked by structure and existing row references; compatible populated tables and replay incur no rebuild or writes. The obsolete `workflow_engine` target was never a table in the historical model. Missing/wrong FKs, orphan rows, unsupported table/view collisions, and ambiguous optional dependencies stop with fixed backup-reconciliation guidance. Downgrade refuses destructive ownership assumptions.
  - Added actual-revision disposable SQLite and opt-in MySQL 8.4 regressions. No configured/production database was used; full version-table traversal and catalog migration remain later Task 9 work.

- **MySQL glossary shadow-copy checkpoint (2026-09-24, Task 9D4B5)**:
  - The published revision now preflights historical tables, collation collisions, triggers, incoming FKs and reserved names before MySQL DDL. It copies exact raw rows and unique terminology mappings to a validated staging table, then rechecks all source metadata and schema under table locks before an atomic two-table rename. The original glossary remains as a named backup and terminology memory remains untouched.
  - A migration-owned audit table records a digest and count of the preserved backup before the rename. Read-only replay requires a valid backup and matching audit while allowing later edits to the active glossary. Interrupted pre-cutover attempts leave the original table active and reserved objects for manual reconciliation; the revision never silently deletes them. Disposable MySQL 8.4 regressions cover preservation, conflicts, concurrent row/metadata/schema/trigger changes, interrupted-state names and replay integrity. Configured/production databases and full chain cutover have not been exercised.
...
- **Frozen historical glossary prerequisites (2026-09-24, Task 9D4A)**:
  ...
  - Verified historical PK/FK/index/default/nullability behavior on SQLite and MySQL-shaped reflection/DDL offline, including boolean aliases. Updated the knowledge base. No application ORM/API/configuration change, no published glossary revision/lineage edit, and no glossary text normalization, merge, or deletion. Task 9D4B retains the published revision's data-handling review; full-chain/version-table traversal and live MySQL/production cutover remain later work.

- **Guarded voice timeline (2026-09-24, Task 9D3B2)**:
  - Review fix: SQLite permanent and temporary trigger preflight compares target table names with `COLLATE NOCASE`, matching identifier resolution. Uppercase/mixed-case trigger declarations can no longer bypass rejection and overwrite preserved originals during backfill. Added failing-first populated no-write regressions for all three spellings in both catalogs.
  - Guarded the published `20260916_character_voice_timeline` revision without changing ancestry or frozen `8ae030a` schema. Complete pre-timeline and published/startup post-timeline profiles are validated before any write; partial/mixed schemas, later collisions, incompatible constraints/indexes and SQLite shadows/triggers refuse with fixed reconciliation guidance.
  - Reused B1's strict prerequisite definitions through shared read-only namespace validation, bounded SQL-default matching and unique-constraint checks. Pre-timeline creates missing children before direct additive columns/indexes and profile/pool tables; no populated-table rebuild. Original timestamps backfill only when NULL, existing values survive, replay avoids redundant updates, and downgrade refuses destructive ownership assumptions.
  - Added independent literal fixtures and actual-revision SQLite tests in `backend/tests/test_voice_timeline_alembic_link.py`, plus offline MySQL DDL/real-parser boolean and unique-index reflection coverage. Updated the knowledge base. Affected migration/helper, tests/fixture and documentation only; no runtime API/ORM/configuration change.
  - No configured/production DB, real keys/provider/network, stamp, push or full-chain claim. Version-table width/traversal remains Task 9D6; later glossary/FK/catalog links, live MySQL, arbitrary production schemas and upgrade-head remain pending.

- **Frozen voice prerequisites (2026-09-24, Task 9D3B1)**:
  - Review fix: require reflected MySQL `AUTO_INCREMENT` on the frozen translation-segment integer ID and reject unexpected auto-increment flags elsewhere. Added failing-first, offline MySQL-shaped `ensure()` rejection and compatible-repeat tests using SQLAlchemy's real SHOW CREATE parser; no SQL executes in either case.
  - Added explicit-connection `backend/alembic/voice_prerequisites.py`, frozen from `8ae030a^`: only historical speaker mappings and translation segments, with no SQL defaults. Both prerequisite tables and existing parent identities are preflighted before any DDL; partial groups, schema/index mismatches and namespace collisions fail without writes. Existing rows and repeat calls are preserved; downgrade refuses ambiguous ownership.
  - Added independent SQL fixtures and isolated tests in `backend/tests/test_voice_prerequisites.py`, covering populated preservation, exact PK/FK/index/default contracts, SQLite rowid allocation/cascades, column/index collation and temporary-shadow rejection, and offline MySQL DDL compilation. Updated the knowledge base.
  - Published timeline revision remains unchanged. No new voice columns, profile/pool tables, backfill, ORM/API/configuration changes, configured database, keys, network, stamp or push. Full-chain/version-table traversal, upgrade-head, live MySQL and production compatibility remain unproven; wiring is Task 9D3B2.

- **Historical D1/D2 primary-key semantics (2026-09-23, Task 9D2X)**:
  - Review fix: normalize only reflected SQLite collation-name spelling, accepting equivalent `binary`/`BiNaRy` column/table PK declarations while retaining exact ID and ascending-order checks. Added populated actual-revision regressions for D1 and both D2 entrypoints.
  - Added migration-owned SQLite identity validation shared by the initial and progress prerequisites: BINARY/ascending string PKs, historical integer rowid alias for `segments`, and default/explicit ABORT conflict policy. Incompatible or ambiguous forms fail before any DDL/DML with the existing fixed reconciliation error; no tables are rebuilt and no rows are rewritten.
  - Preserved equivalent column/table PK declarations and quoted/commented syntax, portable MySQL table-level PK reflection, revision ancestry, and the existing D2 preflight of both groups. Added actual-revision populated SQLite rejection/preservation tests, disposable rollback probes of REPLACE cascade loss, valid rowid allocation, and offline MySQL reflection checks. Updated the knowledge base.
  - Affected `backend/alembic/historical_sqlite_pk.py`, the initial revision, `progress_prerequisites.py`, and `test_historical_primary_key_semantics.py`. No ORM/API/configuration change or configured database/key/network/stamp/push. Full-chain/version-table traversal, live MySQL, arbitrary production schemas and upgrade-head remain unproven; character voice and later chain work remain pending.

- **Historical TikTok migration link (2026-09-23, Task 9D3A)**:
  - Review fix: SQLite primary-key preflight now validates binary collation and ascending order, and checks actual conflict clauses for default/explicit ABORT semantics. Populated NOCASE/DESC/non-ABORT primary keys fail without writes; equivalent column/table constraint syntax, comments and quoted constraint names remain accepted. Added duplicate-ID rejection and unchanged-row regressions.
...
- **Canonical Provider and Key Pool Settings UI (2026-09-23, Task 8C)**:
  ...
  - Replaced the legacy hardcoded AI & API provider/key manager with a canonical provider inventory and masked Key Pool. Key add, enable/disable, delete, and explicit model refresh use `/api/ai` endpoints; partial, failed, stale, and unsupported refresh outcomes remain visible without claiming catalog cleanup.
  - Added backend-derived `keyless` to the provider read DTO and hid key controls for keyless services. Removed unsupported priority/quota/usage/test fields and the legacy custom-provider creation form. Social and System tabs remain available; generic custom-provider creation awaits a canonical API.
  - Added synthetic React DOM and disposable SQLite coverage, updated the knowledge base, and ran frontend tests/build plus focused backend tests. No live provider or production database access.

- **Canonical Function Model Picker (2026-09-23, Task 8B)**:
  - Review fix: capability-filtered model list rows now include backend-calculated `selectable` and capability-specific access scope. The picker trusts this field, so image-only keyless Pollinations/local-image rows are disabled for STT while legitimate keyless Image/TTS rows remain selectable. Unfiltered catalog/detail keep their prior provider-wide scope and report `selectable: null`. Added socket-guarded disposable SQLite and mocked DOM regressions; no schema or provider traffic.
...
- **Unified ANALYZE/TRANSLATE catalog routing and checkpoint hydration (2026-09-23, Task 6E2A)**:
  ...
  - Added isolated synthetic SQLite/provider/media regressions for concurrent exact model/key requests, fallback, missing defaults, transaction-free provider boundary, fresh-engine resume/retry, malformed checkpoint, and upstream-secret redaction. No schema/API/UI/live provider/production database changes; updated the knowledge base.
...
- **TTS provider request boundary (2026-09-23, Task 6D1)**:
  ...
  - Added mocked concurrent model/key/voice, legacy, mismatch, failure, and keyless tests. Google canonical routing still lacks a voice discovery/access bridge and is not end-to-end usable. No database, API, UI, Studio, Unified, or live provider changes.

- **Studio Auto translation canonical cutover (2026-09-23, Task 6C)**:
  - Review follow-up: HTTP-success responses are now checked inside each route target attempt. Malformed JSON advances immediately; incomplete or echoed initial output retains targeted recovery, while exhausted recovery/correction output advances to the next compatible model. Glossary correction also advances when a target repeats an invalid term. Empty `lines` envelopes stay empty instead of being reinterpreted as text. Semantic failures use an allowlisted `invalid_output` route code and do not retry the same target or expose raw model output.
  - Studio Phase 1 translation now passes its catalog session factory into the translator. Populated Gemini/OpenAI canonical state requires a configured TRANSLATION default and invokes immutable catalog routes for initial batches, missing-ID recovery, echo/glossary correction, and recursive split batches. Same-provider and cross-provider fallback use request-local remote model IDs and decrypted keys; incompatible models and unsupported adapters are skipped by the route. The uninitialized catalog retains legacy `.env` compatibility.
  - Removed translation's shared provider `_resolved_model_id` writes. Legacy calls now pass their resolved model as a local argument. Canonical batches use persisted input-token metadata where present, a 6 KiB fallback ceiling, a 512-token response reserve, and UTF-8 byte length as a conservative bound; every outbound prompt is checked. Canonical outer retries are limited to one pass around bounded route attempts, preserving numbered JSON mapping, glossary checks, and echo recovery.
  - Added isolated SQLite, synthetic-key, mocked HTTP/adapter regressions for exact model/key, failover order, incompatible/unknown models, concurrent jobs, recovery paths, context bounds, and secret-safe failures. Updated the knowledge base. No schema, public API, UI, TTS, visual, Unified, or legacy workflow entry changes.

- **Studio Auto STT canonical cutover (2026-09-23, Task 6B)**:
  - Review follow-up: a global refresh record without Gemini/OpenAI catalog state no longer disables legacy STT. Studio's whole-call route timeout now scales with the possible chunk count and is capped at six hours per attempt, so valid calls past 30 seconds are not cancelled by the generic route default. Added failing-first refresh and injected-clock timeout regressions; retained existing per-request timeouts and route retry bounds.
  - Studio Phase 1 STT now builds and invokes the canonical route from the configured STT default, forwarding each target's exact model and decrypted key to request-local Gemini/OpenAI adapters. Canonical fallback ignores the legacy `ENABLE_OPENAI_FALLBACK` flag, tries same-provider candidates first, and reports sanitized classified failures.
...
- **Request-local LLM/STT provider boundary (2026-09-23, Task 6A)**:
  ...
  - Gemini generation and validation place keys in `x-goog-api-key` headers instead of URLs. LLM/STT HTTP errors and Gemini STT parser diagnostics exclude raw provider bodies, request URLs, and credentials. `ai_routing.classify_failure` now reads `PipelineError.http_status` to preserve route fallback classification.
  - Added mocked concurrent HTTP regressions and focused STT/route compatibility tests. Updated the knowledge base. No schema, API, UI, workflow consumer wiring, live key, or production database change.

- **Canonical capability and routing contract (2026-09-23, Task 5)**:
  - Review follow-up: partial curated annotations now leave omitted capabilities unknown/selectable; only `KNOWN` or `COMPLETE` annotations treat omissions as negatives. A legacy `model_id="default"` setting now prioritizes the configured provider or raises a visible configuration error when it has no compatible accessible model.
...
- Review follow-up: a Visual Gender default referencing a canonical catalog model ID now activates the route even if its provider is neither Gemini nor OpenAI. Unrelated provider-only catalog/key state still preserves the legacy migration hold. Red/green regressions cover discovered and system-sourced Anthropic defaults; key-only unrelated, Edge-only, and Anthropic-access unsupported-adapter checks were added. Final focused rerun: 107 passed. An explicit per-function migration marker remains a Task 9 concern.
- **Unified DUB canonical TTS and resumable clips (2026-09-23, Task 6E2B)**:
  - Unified `DubStage` now uses the catalog TTS default, compatible same-/cross-provider fallback, request-local model/key credentials, voice-pool language/gender filtering, and pinned saved speaker voices. Unsupported Google Cloud catalog targets are excluded until voice-access discovery exists; keyless Edge remains supported. The exact unmigrated legacy Edge default retains its positional compatibility path.
  - Project-scoped clip sidecars and checkpoint audio metadata retain provider/model/key-ID/voice/text/default identity without decrypted keys; a changed default or voice regenerates clips. Failed generation results surface classified redacted errors; provider/media calls occur outside catalog sessions with bounded durations. Updated the knowledge base and added synthetic SQLite/provider tests. No schema/API/UI or production data change.
...
- **Auxiliary terminology LLM routing (2026-09-23, Task 6E3A1)**:
  - Independent-review fixes: strict canonical parsing now rejects null/wrong/structurally empty term payloads while accepting a genuine empty list; prompt budgets are per target, so a tiny backup cannot prevent a usable default from running; legacy migration-hold error logs use only classified codes. Added failing-first regressions and retained the no-outbound-network fixture.
  - Studio and Unified terminology extraction now forward canonical catalog sessions and route LLM calls through the translation default, exact per-request model/key, bounded retries/timeouts, same/cross-provider fallback, and classified malformed-output/unsupported-adapter handling. A schema with no canonical LLM state retains the explicit legacy migration hold.
  - Bounded prompt bytes by available model input metadata and removed raw source/suggested term values from terminology rejection logs. Added isolated synthetic-key SQLite tests for fallback, concurrent jobs, invalid default, missing adapter, prompt bounds, and actual Studio/Unified entry wiring; no live provider calls, schema/API/UI changes, or production database access.
  - Updated the project knowledge base. QC/SEO, character mapping, and thumbnail LLM consumers remain separate follow-up batches.
- **Editor QC/SEO canonical LLM routing (2026-09-23, Task 6E3A2)**:
  - Independent-review fixes: Unified PUBLISH now forwards normalized frozen project SEO settings (including string-valued AI-off), project title and context video/episode identity; creator defaults survive. Canonical SEO rejects all-empty AI description/tags for a nonempty transcript when those fields are enabled, advancing fallback; explicitly disabled AI fields may remain empty. Added actual stage and fallback regressions under the no-network guard.
  - Added a small shared editor JSON route boundary using the translation LLM default, per-target prompt budgets, request-local model/key, compatible fallback, strict output validation, bounded retries/timeouts and sanitized errors. QC and SEO now use it after catalog activation; malformed/exhausted QC cannot produce a default passing score.
  - Editor QC/SEO endpoints and Unified PUBLISH pass the canonical session factory. Preserved non-AI QC, user SEO defaults and explicit AI-off behavior. Legacy Gemini calls remain only for uninitialized catalogs, now with header credentials and safe log codes. Fixed a missing `json` import on the actual QC endpoint exposed by the entry test.
  - Added isolated SQLite/synthetic-key, actual endpoint/stage seam, concurrent job, semantic fallback and no-network regression tests. No schema/UI, YouTube upload, live provider or production database changes. Updated the knowledge base.
- **Character mapping and thumbnail analysis LLM routing (2026-09-23, Task 6E3B)**:
  - Studio Character Mapping, thumbnail REST routes, Studio automatic thumbnail and Unified PRODUCE/PUBLISH now forward catalog sessions to the reviewed JSON route boundary. Canonical LLM requests use the configured translation default, exact model/key, per-target prompt budgets, compatible fallback and strict semantic validation. Uninitialized catalogs retain legacy paths until migration.
  - Preserved stable speaker mapping/user-confirmed profiles and all thumbnail image/storage behavior. Malformed legacy character candidates become unresolved instead of crashing; character/thumbnail LLM logs no longer include raw exception or title text. Added isolated synthetic SQLite, no-network, concurrent-request, semantic-fallback and actual entry-point tests. No schema, UI, live API or production database changes.
- **Task 6E3B independent-review fixes (2026-09-23)**:
  - Unknown-only speaker candidates trigger compatible fallback; confirmed name/role are returned to Studio; nullable voice-only mapping rows retain generated character IDs. Mapping now obtains the canonical LLM answer before opening caller DB reads, and thumbnail avoids a pre-analysis refresh after committing its status row, so a one-connection pool can serve request-local catalog sessions. Added failing-first regressions for all four findings and reran the no-network focused suite.
- **Keyed image adapter boundary (2026-09-23, Task 6E4A/A)**:
  - OpenAI/fal adapters now accept exact request-local catalog model/key pairs. Canonical generation handles OpenAI URL/base64 images and bounded Fal queue polling without mutable provider state. Both keyed adapters' still-reachable legacy branches now sanitize upstream errors and use the same bounded HTTPS/public-image download; optional output paths are limited to `DATA_DIR`.
  - Added MockTransport/synthetic-key tests for exact model/key, CDN auth isolation, response/media caps, MIME/magic, private URL/redirect rejection, Fal polling host, safe legacy behavior and output paths. Caller routing in ThumbnailService is the next reviewed batch; no image generation API, storage or database schema change in this adapter batch.
  - Independent-review fixes: image bytes now require Pillow structural verification plus full pixel decode under dimension/pixel caps; corrupt/truncated/decompression-bomb payloads fail. Fal queue polling has a 75-second local deadline, and submitted-but-pending/uncertain work emits terminal `RoutePending` rather than triggering a second billed submission or cross-provider fallback. Router propagation and pre-POST cancellation behavior have isolated regressions; Task 6E4A/B must set a longer attempt budget and surface pending explicitly.
- **Keyless image adapter hardening (2026-09-23, Task 6E4A/B checkpoint)**:
  - Pollinations and local/Picsum downloads now use bounded per-hop public HTTPS redirects, capped/decoded image payloads and no authorization headers. Pollinations optional output paths are confined to `DATA_DIR`; Local rejects caller output paths and uses cleaned-up unique task-data temporary files for FFmpeg. Provider logs/errors no longer include prompt excerpts or raw exceptions. MockTransport/no-network tests cover redirect success/private-chain rejection, malformed/oversized image, output path, and temporary-file cleanup. No API or database migration in this checkpoint.
- **Thumbnail image catalog caller cutover (2026-09-23, Task 6E4A/B)**:
  - Canonical image generation now uses the configured catalog default and compatible same-/cross-provider targets with exact request-local model/key, one attempt per target, and a 150-second per-attempt budget. The unmigrated legacy Pollinations setting remains explicitly gated. Keyless Pollinations/local models require `source=system`; forged keyless route targets are rejected. Explicit model overrides are validated, while Studio legacy hints and implicit regenerate history permit the current default to win.
  - Submitted-but-unresolved Fal work yields visible `provider_pending`, blocks sequential duplicate requests, and preserves the old active thumbnail. Images are fully validated and stored with actual format, MIME, dimensions and exact provider/model identity. Storage target IDs must be safe single path components, and temporary files are cleaned after writes/uploads. Endpoint errors and logs use fixed/classified messages.
  - Added additive Alembic `20260923_thumbnail_model_length` to widen `video_thumbnails.model` to 255; downgrade refuses long IDs. Disposable SQLite migration and MySQL offline SQL were checked, not production/full historical migration. Guarded synthetic/offline tests cover routing, pending, overrides, legacy hold, migration, media and entry points. Keyless checkpoint independent review was clean; this caller batch awaits scoped review. See knowledge base for pending reconciliation, concurrent-launch and overall-deadline limitations.
- **Task 6E4A/B review fixes (2026-09-23)**:
  - Partial explicit regenerate overrides no longer inherit the unrelated old provider/model. Same-second thumbnail writes include a unique thumbnail ID in the storage key, so a failed later upload cannot mutate the old active image. Added failing-first endpoint, service and real isolated-local-storage regressions; guarded suite: 122 passed.
  - Legacy compatibility follow-up: old provider/model are passed separately and used only if no canonical image route is active. Legacy partial regenerate keeps the unsupplied old field, while catalog partial regenerate still uses its current configured default. Failing-first real endpoint/service regressions added; guarded suite: 126 passed.
  - Failure-record follow-up: commit the resolved legacy provider/model before adapter invocation so failed partial regenerations retain their intended selection for display and later retry. Failing-first persisted-row and sanitized-error regressions added; guarded suite: 128 passed.
- **Keyed video adapter boundary (2026-09-23, Task 6E4B/A)**:
  - Fal/Kling video providers accept exact request-local catalog model/key targets. Fal Hunyuan uses its documented frame-based schema; Kling canonical text-to-video sends the exact documented `model_name`. Heterogeneous or undocumented video model IDs are rejected without substitution.
  - Added bounded queue polling, JSON/MP4 downloads, provider/media URL checks, no media authorization forwarding, and atomic `DATA_DIR` writes. Accepted/uncertain work is terminal pending; known failed jobs and explicit rejected submissions expose safe non-pending outcomes. Legacy positional calls retain KeyManager use and rotate only on definitive auth/quota/rate rejection; errors and result metadata omit upstream bodies, prompt previews and masked keys. Updated the knowledge base. No API/DB/UI/orchestrator/Settings or live provider change.
- **Task 6E4B/A review fixes (2026-09-23)**:
  - Kling malformed or absent HTTP-200 submit codes now remain terminal pending; only typed integer nonzero is definitive rejection. Fal/Kling submit streaming and parsing now have an overall 30-second deadline, closing a trickle-response gap.
  - MP4 files are checked structurally for bounded ISO-BMFF boxes, a video track/sample description and nonempty media data before replacing output. Extracted shared billing and legacy rotation policy; added guarded regressions for both providers. Structural checks do not decode frames or establish browser playability; no FFmpeg, fixture or decoder dependency was available offline. No schema, API, UI, caller or live provider change.
- **Task 6E4B/A review fix round 2 (2026-09-23)**:
  - Replaced structural-only MP4 acceptance with fail-closed, bounded FFprobe metadata/packet checks and FFmpeg one-frame decode before atomic output replacement. Reviewer reproductions for zero dimensions, missing sample tables, plain-text media data, and malformed later `moov` child now fail. Tool absence, timeout, excessive output and decode failure yield safe errors. No production dependency or caller change; the offline environment has no FFmpeg/FFprobe or playable fixture, so real-tool malformed-media coverage is conditional.
...
- **Unified script-to-video catalog caller cutover (2026-09-23, Task 6E4B/B2)**:
  - The workflow caller and project precheck now share an exact catalog-ID activation gate. Seeded legacy Kling/Fal defaults and projects without a canonical default retain their historical provider route; invalid explicit selections fail visibly.
  - Canonical Fal/Kling calls use ordered catalog targets and decrypted request-local keys with one submission attempt per target. Completed media is staged under `DATA_DIR` and atomically published under `STORAGE_ROOT`; post-success publish/probe failures retain a recovery copy and persist `provider_pending` to prevent a second billable submission.
  - Added disposable SQLite and synthetic adapter coverage for cross/same-provider fallback, concurrent model/key isolation, legacy hold, invalid and retired defaults, pending, publish/probe failures, and the precheck endpoint. No schema, Settings UI/API, Studio, live provider, or production database change. Updated the knowledge base.
  - Review fix: Commit a segment-level pending guard before each billable canonical provider call; definite rejections clear it before fallback, while uncertain outcomes retain it. Startup and resumed runs reconcile a persisted segment guard to project pending before any new work. Local path setup/notifications run before the guard. Added interrupted→resume and persistent DB failure regressions; split the lightweight video boundary error types for strict caller classification while preserving `catalog_media` re-exports.
- **Canonical AI catalog read API (2026-09-23, Task 7A)**:
  - Review fix: unfiltered model pages use SQL COUNT/LIMIT/OFFSET; capability/search pages stream ordered candidates in bounded batches and preserve exact totals. Both load key-access/default rows only for page models. Added 600-model/1,200-edge and filtered-stream disposable SQLite regressions.
  - Added typed read-only `/api/ai` provider, paginated model, model-detail, and function-default GETs with the existing `{success,data}` envelope. Canonical SQLite-backed views expose current key counts, evidence-derived capability compatibility, retired/disabled/default status, and public-catalog versus keyless access scope without credential fields or GET writes.
  - Added disposable SQLite API tests for provider grouping, two-key deduplication, searches/filters, pagination, FULL_UNKNOWN, keyless Edge, invalid defaults, read-only SQL, and fixed safe error responses. Updated the knowledge base. Legacy Settings writes and migration remain Task 7B/9 scope; no schema, production DB, or provider traffic change.
# Canonical Key Result Accounting and Cooldown (2026-09-23, Task 9B2B)

- Review fix: an invocation-local rejected-key set prevents reusing a 429/401/explicit-quota key on another model when result-accounting persistence fails. Added a failing-first synthetic two-model regression and reran the related suites.
...
- Added disposable, network-blocked Settings-to-caller E2E tests for Studio STT, Unified DUB/TTS and legacy Project video. Each adds a synthetic key, discovers a model, selects it through the canonical Function API and verifies the exact model/key reaches a mocked provider and produces the expected output. API and workflow snapshots do not expose the key. No production code or database schema changed in this test batch.
- Offline verification for the current Task 9–10 code: 1,751 backend tests passed (45 skipped, one warning); 30 frontend tests and the build passed; a disposable SQLite/MySQL migration subset passed 52 tests (one skipped). Actual Alembic downgrade/reupgrade across the catalog branch merge passed on empty disposable SQLite/MySQL databases; populated production rollback remains unapproved. The Settings tests stop at routed AI callers rather than final media rendering. Legacy Settings writers, JSON-key adapters and hardcoded archival seeds remain gated by their active compatibility consumers; no configured/production database, live provider, generic stamp or push was used.
...
- **OpenRouter and extensible provider catalog (2026-09-25)**:
  - Added persistent canonical `POST /api/ai/providers` and an AI Provider Catalog button for custom provider profiles. Unknown providers are labeled as lacking an adapter; creation does not imply model execution. OpenRouter is registered as a built-in multimodal provider.
  - OpenRouter key discovery verifies a generation key through authenticated `/api/v1/key` before accepting the public model catalog; management keys stay disabled. A `verified_catalog` result can activate the key without inventing per-model access edges. Discovery requests all output modalities and stores bounded capability, model-scoped voice, and video duration evidence. A failed auxiliary video listing leaves the verified nonvideo catalog usable. Function labels follow positive model evidence; missing fields remain unknown. Exact documented Gemini chat IDs receive conservative audio/vision inference when the Gemini listing omits modality fields.
  - Added request-local OpenRouter workflow adapters for transcription, text/translation, visual analysis, speech, raster image, and asynchronous video generation. Whisper STT requests timed segments; models without timed output retain coarse unresolved-speaker chunks. Studio TTS voices are selected from the configured model's published IDs without assuming language or gender; models with no published voice IDs are not selectable for TTS. Video duration is checked against dedicated model evidence during selection and again before submission; known SVG-only Recraft variants and image-only edit models are excluded from text-to-image selection. No real API key or production database was used in verification.
  - Affected canonical catalog/discovery/routing APIs and services, Studio settings/voice UI, provider bootstrap/runtime adapters, focused tests, and `PROJECT_KNOWLEDGE_BASE.md`. No database schema or environment variable changes.

# Live Audio Translation (2026-09-25)
