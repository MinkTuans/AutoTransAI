- **Studio Auto STT canonical cutover (2026-09-23, Task 6B)**:
  ...
  - Explicit compatibility gate keeps `.env` STT only for schemas without Gemini/OpenAI non-system catalog models or keys, or refresh activity. Keyless system and unrelated TTS rows alone leave legacy STT available; activated catalogs with missing/invalid defaults fail visibly, and absent canonical tables surface migration errors.
