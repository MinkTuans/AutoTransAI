"""
FFmpeg wrapper — audio/video processing operations.

All operations use safe subprocess execution with list arguments.
Never uses shell=True or string interpolation for commands.
"""
...
def concatenate_videos(
    ...
    Uses the concat demuxer for efficient concatenation without re-encoding
    when codecs are compatible, or re-encodes when necessary.
    """
    _check_ffmpeg()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    # Try concat demuxer first (no re-encoding)
    ...
    if result.returncode != 0:
        # Fallback: re-encode for codec compatibility
        logger.warning(
