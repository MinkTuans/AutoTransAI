config.render = {
  ...
  "circular": function (e) {
    ...
    svg.element = `
      <svg xmlns="http://www.w3.org/2000/svg" width="${Math.abs(svg.x2 - svg.x1)}" height="${Math.abs(svg.y2 - svg.y1)}">
        <line x1="${svg.x1}" y1="${svg.y1}" x2="${svg.x2}" y2="${svg.y2}" stroke="${config.ruler.stroke.color}" stroke-width="2"/>
      </svg>
    `;
    /*  */
    if (circle.top.right) {
  ...
  "angular": function (e) {
    ...
    svg.trianlge = {};
    /*  */
    svg.methods.polar2cartesian = function (radius, angle) {
      const b = radius * Math.cos(angle);
      const h = radius * Math.sin(angle);
      const cx = circle.top.left ? svg.x2 : svg.x1;
      const cy = circle.top.left ? svg.y2 : svg.y1;
      const x = circle.bottom.right ? cx + b : (circle.bottom.left ? cx - b : (circle.top.right ? cx + b : cx - b));
      const y = circle.bottom.right ? cy + h : (circle.bottom.left ? cy + h : (circle.top.right ? cy - h : cy - h));
      /*  */
      return {'x': x, 'y': y};
    };
    /*  */
    svg.trianlge.base = Math.abs(svg.x2 - svg.x1);
    svg.trianlge.height = Math.abs(svg.y2 - svg.y1);
    svg.arc.angle = Math.atan(svg.trianlge.height / svg.trianlge.base);
    svg.arc.r = Math.abs(svg.x2 - svg.x1) < 150 ? Math.abs(svg.x2 - svg.x1) / 1.5 : Math.abs(svg.x2 - svg.x1) / 2;
    svg.arc.start = svg.methods.polar2cartesian(svg.arc.r, svg.arc.angle);
    svg.arc.sweep = circle.top.left || circle.bottom.right ? 0 : 1;
    svg.arc.end = svg.methods.polar2cartesian(svg.arc.r, 0);
    svg.arc.large = svg.arc.angle <= 180 ? 0 : 1;
    /*  */
    text.angle = document.createElement("div");
    text.angle.textContent = "(∠) " + Math.round(svg.arc.angle * 180 / Math.PI) + "°";
    svg.element = `
      <svg xmlns="http://www.w3.org/2000/svg" width="${svg.trianlge.base}" height="${svg.trianlge.height}">
        <line x1="${svg.x1}" y1="${svg.y1}" x2="${svg.x2}" y2="${svg.y2}" stroke="${config.ruler.stroke.color}" stroke-width="2"/>
        <line x1="${svg.x1}" y1="${circle.top.left ? svg.y2 : svg.y1}" x2="${svg.x2}" y2="${circle.top.left ? svg.y2 : svg.y1}" stroke="${config.ruler.stroke.color}" stroke-width="5"/>
        <line x1="${circle.top.left ? svg.x1 : svg.x2}" y1="${svg.y1}" x2="${circle.top.left ? svg.x1 : svg.x2}" y2="${svg.y2}" stroke="${config.ruler.stroke.color}" stroke-width="5"/>
        <path d="${["M", svg.arc.start.x, svg.arc.start.y, "A", svg.arc.r, svg.arc.r, 0, svg.arc.large, svg.arc.sweep, svg.arc.end.x, svg.arc.end.y].join(" ")}" fill="none" stroke="${config.ruler.stroke.color}" stroke-width="3"/>
      </svg>
    `;
    /*  */
    if (circle.top.right) {
      if (inrange.sx && inrange.ex && inrange.sy && inrange.ey) {
        ...
        text.c.style.top = visible ? metrics.y : (svg.y1 + text.height) + "px";
        /*  */
        text.angle.style.left = visible ? metrics.w : (svg.x1) + "px";
        text.angle.style.top = visible ? metrics.cc : (svg.y1) + "px";
        /*  */
        text.e.style.left = (svg.x2 - text.width) + "px";
        text.e.style.top = (svg.y2 - 2 * text.height) + "px";
        /*  */
        ...
        text.c.style.top = (center.placement.y + 1 * (text.height + text.spacing)) + "px";
        /*  */
        text.angle.style.left = (center.placement.x + text.width + text.spacing) + "px";
        text.angle.style.top = (center.placement.y + 1 * (text.height + text.spacing)) + "px";
        /*  */
        text.b.style.left = (center.placement.x) + "px";
        text.b.style.top = (center.placement.y + 2 * (text.height + text.spacing)) + "px";
        /*  */
    ...
    if (circle.bottom.right) {
      if (inrange.sx && inrange.ex && inrange.sy && inrange.ey) {
        ...
        /*  */
        text.angle.style.left = visible ? metrics.w : (svg.x1) + "px";
        text.angle.style.top = visible ? metrics.cc : (svg.y1 - 2 * text.height) + "px";
        /*  */
        text.e.style.left = (svg.x2 - text.width) + "px";
        text.e.style.top = (svg.y2) + "px";
        /*  */
    ...
    if (circle.top.left) {
      if (inrange.sx && inrange.ex && inrange.sy && inrange.ey) {
        ...
        text.c.style.top = visible ? metrics.y : (svg.y2 + text.height) + "px";
        /*  */
        text.angle.style.left = visible ? metrics.w : (svg.x2) + "px";
        text.angle.style.top = visible ? metrics.cc : (svg.y2) + "px";
        /*  */
        text.e.style.left = (svg.x2 - text.width) + "px";
        text.e.style.top = (svg.y2) + "px";
        /*  */
    ...
    if (inrange.sx && inrange.ex && inrange.sy && inrange.ey) {
      ...
      config.ruler.element.rect.appendChild(text.b);
      config.ruler.element.rect.appendChild(text.c);
      config.ruler.element.rect.appendChild(text.angle);
      config.ruler.element.rect.appendChild(text.e);
      config.ruler.element.rect.appendChild(text.f);
    } else {
      config.ruler.element.section.appendChild(text.a);
      config.ruler.element.section.appendChild(text.b);
      config.ruler.element.section.appendChild(text.c);
      config.ruler.element.section.appendChild(text.angle);
      config.ruler.element.section.appendChild(text.e);
      config.ruler.element.section.appendChild(text.f);
      /*  */
      config.ruler.element.rect.setAttribute("visible", false);
