  ...
  var config = {
    ...
    "action": {
      ...
      "placement": function (e) {
        config.options.adjust = e.adjust;
        config.options.scroll = e.scroll;
        config.options.linear = e.linear;
        config.options.angular = e.angular;
      ...
      "is": {
        ...
        "dark": function (e) {
          try {
            ...
            /*  */
            for (let i = 0; tmp && i < 10; i++, tmp = tmp.parentElement) {
              const color = window.getComputedStyle(tmp).backgroundColor;
              if (color !== "rgba(0, 0, 0, 0)" && color !== "transparent") {
                return config.page.is._dark(color);
              }
            }
            /*  */
            return config.page.is._dark(null);
          } catch (e) {
            return config.page.is._dark(null);
          }
        },
        "_dark": function (e) {
          try {
            ...
            /*  */
            return config.page.is._mediaDark.matches;
          } catch (e) {
            return config.page.is._mediaDark.matches;
          }
        }
    ...
    "ruler": {
      ...
      "input": {
        ...
        "init": function (e) {
          if (e.cancelable) e.preventDefault();
          const flag = config.options.adjust && config.page.is.dark(e);
          /*  */
          config.ruler.active = true;
          config.ruler.stroke.color = flag ? "rgb(255 255 255 / 50%)" : "rgb(0 0 0 / 50%)";
          /*  */
        ...
        "move": function (e) {
          ...
          if (action) {
            if (config.ruler.element.rect && config.ruler.element.circle) {
              ...
              args.placement = config.options.colorful ? "colorful" : (config.options.grayscale ? "grayscale" : (config.options.circular ? "circular" : (config.options.linear ? "linear" : (config.options.angular ? "angular" : "default"))));
              /*  */
              ...
              args.metrics.cc = (args.height / 2 - (args.text.height / 2) - args.text.height - 2 * args.text.adjustment) + "px";
              /*  */
              ...
              } else if (config.options.angular) {
                config.render.angular(args);
